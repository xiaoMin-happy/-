...
- HTTP是一个客户端（用户）和服务器端（网站）之间进行请求和应答的标准，基于“请求与响应”模式的、无状态的应用层协议

- 通过使用网页浏览器、网络爬虫或者其他工具，客户端可以向服务器上的指定端口（默认端口为80）发起一个HTTP请求。这个客户端称为客户代理（user agent）
...
HTTP协议对资源的操作的六个方法（即六个HTTP请求），是与Requests库的六个主要方法一一对应。
